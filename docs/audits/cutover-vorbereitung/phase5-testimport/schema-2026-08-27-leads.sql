...
-- Erzeugt 2026-08-27T15:18:03.941Z aus dem Live-Katalog (scripts/phase5-schema-export.js)
...
-- Schemata sind auf die Plattform abgebildet (Quelle public -> leads,
-- Quelle analytics_internal -> leads_analytics); Restkontrolle erzwungen.

\set ON_ERROR_STOP on
BEGIN;

-- Superuser-Teil. AUTHORIZATION haelt das Schema-Eigentum bei leads_owner,
-- auch wenn ein Superuser den Import faehrt. pgcrypto: Paritaet zur Quelle;
-- die Funktionsruempfe selbst brauchen nur pg_catalog (gen_random_uuid, hashtext).
...
-- Rollenmodell-Vertrag: jede Migration beginnt mit SET ROLE leads_owner -
-- sonst gehoeren die Objekte dem Zugang und die Standardrechte greifen nicht.
SET LOCAL ROLE leads_owner;
-- Unqualifizierte Verweise in View-/Constraint-/Trigger-Definitionen loesen
-- ueber den search_path in die Zielschemata auf.
SET LOCAL search_path TO leads, leads_analytics;
...
CREATE SEQUENCE leads.quiz_sessions_id_seq AS bigint INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START WITH 1 CACHE 1;
CREATE SEQUENCE leads.tracking_sessions_id_seq AS bigint INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START WITH 1 CACHE 1;
CREATE SEQUENCE leads.tracking_events_id_seq AS bigint INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START WITH 1 CACHE 1;
CREATE SEQUENCE leads.tracking_video_progress_id_seq AS bigint INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START WITH 1 CACHE 1;
CREATE SEQUENCE leads.lead_profiles_id_seq AS bigint INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START WITH 1 CACHE 1;

CREATE TABLE leads.lead_state (
...
CREATE TABLE leads.lead_video_progress (
  lead_hash text NOT NULL,
...
CREATE TABLE leads.lead_answers_current (
  lead_hash text NOT NULL,
...
CREATE TABLE leads.lead_sync_outbox (
...
CREATE TABLE leads.lead_profiles (
...
CREATE TABLE leads.nurture_sequences (
...
CREATE TABLE leads.nurture_runs (
...
CREATE TABLE leads.nurture_subject_states (
  project_key text NOT NULL,
  sequence_key text NOT NULL,
  lead_hash text NOT NULL,
  status text NOT NULL,
...
CREATE TABLE leads.tracking_sessions (
...
CREATE TABLE leads.tracking_events (
...
CREATE TABLE leads.tracking_video_progress (
...
CREATE TABLE leads.quiz_sessions (
...
CREATE TABLE leads.lead_migration_unresolved (
  id bigint GENERATED ALWAYS AS IDENTITY NOT NULL,
...
CREATE TABLE leads.lead_contact_crm (
...
CREATE TABLE leads_analytics.event_daily (
...
CREATE TABLE leads_analytics.refresh_runs (
...
ALTER TABLE leads_analytics.event_daily ADD CONSTRAINT event_daily_pkey PRIMARY KEY (event_day, event_name, source_app, funnel_key, member_id);
ALTER TABLE leads_analytics.refresh_runs ADD CONSTRAINT refresh_runs_pkey PRIMARY KEY (run_id);
ALTER TABLE leads.app_config ADD CONSTRAINT app_config_pkey PRIMARY KEY (key);
ALTER TABLE leads.lead_answers_current ADD CONSTRAINT lead_answers_current_pkey PRIMARY KEY (lead_hash, question_ref);
ALTER TABLE leads.lead_contact_crm ADD CONSTRAINT lead_contact_crm_pkey PRIMARY KEY (lead_hash);
ALTER TABLE leads.lead_events ADD CONSTRAINT lead_events_pkey PRIMARY KEY (event_id);
ALTER TABLE leads.lead_migration_unresolved ADD CONSTRAINT lead_migration_unresolved_pkey PRIMARY KEY (id);
ALTER TABLE leads.lead_profiles ADD CONSTRAINT lead_profiles_pkey PRIMARY KEY (id);
ALTER TABLE leads.lead_state ADD CONSTRAINT lead_state_pkey PRIMARY KEY (lead_hash);
ALTER TABLE leads.lead_sync_outbox ADD CONSTRAINT lead_sync_outbox_pkey PRIMARY KEY (id);
ALTER TABLE leads.lead_video_progress ADD CONSTRAINT lead_video_progress_pkey PRIMARY KEY (lead_hash, video_step);
ALTER TABLE leads.nurture_runs ADD CONSTRAINT nurture_runs_pkey PRIMARY KEY (id);
ALTER TABLE leads.nurture_sequences ADD CONSTRAINT nurture_sequences_pkey PRIMARY KEY (project_key, sequence_key);
ALTER TABLE leads.nurture_subject_states ADD CONSTRAINT nurture_subject_states_pkey PRIMARY KEY (project_key, sequence_key, lead_hash);
ALTER TABLE leads.quiz_sessions ADD CONSTRAINT quiz_sessions_pkey PRIMARY KEY (id);
ALTER TABLE leads.tracking_events ADD CONSTRAINT tracking_events_pkey PRIMARY KEY (id);
ALTER TABLE leads.tracking_sessions ADD CONSTRAINT tracking_sessions_pkey PRIMARY KEY (id);
ALTER TABLE leads.tracking_video_progress ADD CONSTRAINT tracking_video_progress_pkey PRIMARY KEY (id);
ALTER TABLE leads.lead_events ADD CONSTRAINT lead_events_event_uid_key UNIQUE (event_uid);
ALTER TABLE leads.lead_migration_unresolved ADD CONSTRAINT lead_migration_unresolved_source_table_source_id_key UNIQUE (source_table, source_id);
ALTER TABLE leads.lead_profiles ADD CONSTRAINT lead_profiles_profile_key_key UNIQUE (profile_key);
ALTER TABLE leads.lead_state ADD CONSTRAINT lead_state_client_seed_key UNIQUE (client_seed);
ALTER TABLE leads.nurture_runs ADD CONSTRAINT nurture_runs_project_key_sequence_key_execution_id_key UNIQUE (project_key, sequence_key, execution_id);
ALTER TABLE leads.quiz_sessions ADD CONSTRAINT quiz_sessions_hash_key UNIQUE (hash);
ALTER TABLE leads.tracking_events ADD CONSTRAINT tracking_events_event_id_key UNIQUE (event_id);
ALTER TABLE leads.tracking_sessions ADD CONSTRAINT tracking_sessions_session_hash_key UNIQUE (session_hash);
ALTER TABLE leads.tracking_video_progress ADD CONSTRAINT tracking_video_progress_session_video_key_key UNIQUE (session_video_key);
ALTER TABLE leads_analytics.event_daily ADD CONSTRAINT event_daily_distinct_leads_check CHECK ((distinct_leads >= 0));
ALTER TABLE leads_analytics.event_daily ADD CONSTRAINT event_daily_event_count_check CHECK ((event_count >= 0));
ALTER TABLE leads_analytics.refresh_runs ADD CONSTRAINT refresh_runs_status_check CHECK ((status = ANY (ARRAY['complete'::text, 'failed'::text])));
ALTER TABLE leads.lead_profiles ADD CONSTRAINT lead_profiles_last_completed_video_step_check CHECK (((last_completed_video_step >= 0) AND (last_completed_video_step <= 3)));
ALTER TABLE leads.lead_state ADD CONSTRAINT lead_state_lead_hash_check CHECK ((lead_hash ~ '^qz_[a-zA-Z0-9_]+$'::text));
ALTER TABLE leads.lead_state ADD CONSTRAINT lead_state_ref_type_check CHECK ((ref_type = ANY (ARRAY['member'::text, 'referral_code'::text, 'campaign'::text, 'unknown'::text])));
ALTER TABLE leads.lead_sync_outbox ADD CONSTRAINT lead_sync_outbox_attempts_check CHECK ((attempts >= 0));
ALTER TABLE leads.lead_sync_outbox ADD CONSTRAINT lead_sync_outbox_max_attempts_check CHECK ((max_attempts > 0));
ALTER TABLE leads.lead_sync_outbox ADD CONSTRAINT lead_sync_outbox_status_check CHECK ((status = ANY (ARRAY['pending'::text, 'processing'::text, 'done'::text, 'failed'::text, 'dead'::text])));
ALTER TABLE leads.lead_video_progress ADD CONSTRAINT lead_video_progress_duration_seconds_check CHECK ((duration_seconds >= 0));
ALTER TABLE leads.lead_video_progress ADD CONSTRAINT lead_video_progress_max_playhead_percent_check CHECK (((max_playhead_percent >= (0)::numeric) AND (max_playhead_percent <= (100)::numeric)));
ALTER TABLE leads.lead_video_progress ADD CONSTRAINT lead_video_progress_max_unique_watched_percent_check CHECK (((max_unique_watched_percent >= (0)::numeric) AND (max_unique_watched_percent <= (100)::numeric)));
ALTER TABLE leads.lead_video_progress ADD CONSTRAINT lead_video_progress_seek_count_check CHECK ((seek_count >= 0));
ALTER TABLE leads.lead_video_progress ADD CONSTRAINT lead_video_progress_unique_watched_seconds_check CHECK ((unique_watched_seconds >= 0));
ALTER TABLE leads.lead_video_progress ADD CONSTRAINT lead_video_progress_video_step_check CHECK ((video_step = ANY (ARRAY[1, 2, 3])));
ALTER TABLE leads.nurture_runs ADD CONSTRAINT nurture_runs_candidates_checked_check CHECK ((candidates_checked >= 0));
ALTER TABLE leads.nurture_runs ADD CONSTRAINT nurture_runs_execution_id_check CHECK (((length(TRIM(BOTH FROM execution_id)) >= 1) AND (length(TRIM(BOTH FROM execution_id)) <= 160)));
ALTER TABLE leads.nurture_runs ADD CONSTRAINT nurture_runs_failed_count_check CHECK ((failed_count >= 0));
ALTER TABLE leads.nurture_runs ADD CONSTRAINT nurture_runs_sent_count_check CHECK ((sent_count >= 0));
ALTER TABLE leads.nurture_runs ADD CONSTRAINT nurture_runs_skipped_count_check CHECK ((skipped_count >= 0));
ALTER TABLE leads.nurture_runs ADD CONSTRAINT nurture_runs_status_check CHECK ((status = ANY (ARRAY['running'::text, 'success'::text, 'failed'::text])));
ALTER TABLE leads.nurture_runs ADD CONSTRAINT nurture_runs_telemetry_version_check CHECK ((telemetry_version > 0));
ALTER TABLE leads.nurture_sequences ADD CONSTRAINT nurture_sequences_channel_check CHECK ((channel = ANY (ARRAY['email'::text, 'sms'::text, 'whatsapp'::text, 'push'::text])));
ALTER TABLE leads.nurture_sequences ADD CONSTRAINT nurture_sequences_expected_interval_minutes_check CHECK (((expected_interval_minutes >= 5) AND (expected_interval_minutes <= 10080)));
ALTER TABLE leads.nurture_sequences ADD CONSTRAINT nurture_sequences_project_key_check CHECK ((project_key ~ '^[a-z0-9_]{2,50}$'::text));
ALTER TABLE leads.nurture_sequences ADD CONSTRAINT nurture_sequences_sequence_key_check CHECK ((sequence_key ~ '^[a-z0-9_]{2,60}$'::text));
ALTER TABLE leads.nurture_sequences ADD CONSTRAINT nurture_sequences_telemetry_version_check CHECK ((telemetry_version > 0));
ALTER TABLE leads.nurture_subject_states ADD CONSTRAINT nurture_subject_states_lead_hash_check CHECK (((length(TRIM(BOTH FROM lead_hash)) >= 3) AND (length(TRIM(BOTH FROM lead_hash)) <= 160)));
ALTER TABLE leads.nurture_subject_states ADD CONSTRAINT nurture_subject_states_occurrence_count_check CHECK ((occurrence_count > 0));
ALTER TABLE leads.nurture_subject_states ADD CONSTRAINT nurture_subject_states_status_check CHECK ((status = ANY (ARRAY['sent'::text, 'skipped'::text])));
ALTER TABLE leads.nurture_subject_states ADD CONSTRAINT nurture_subject_states_telemetry_version_check CHECK ((telemetry_version > 0));
ALTER TABLE leads.lead_answers_current ADD CONSTRAINT lead_answers_current_lead_hash_fkey FOREIGN KEY (lead_hash) REFERENCES lead_state(lead_hash) ON DELETE CASCADE;
ALTER TABLE leads.lead_events ADD CONSTRAINT lead_events_lead_hash_fkey FOREIGN KEY (lead_hash) REFERENCES lead_state(lead_hash) ON DELETE CASCADE;
ALTER TABLE leads.lead_migration_unresolved ADD CONSTRAINT lead_migration_unresolved_resolved_lead_hash_fkey FOREIGN KEY (resolved_lead_hash) REFERENCES lead_state(lead_hash) ON DELETE SET NULL;
ALTER TABLE leads.lead_sync_outbox ADD CONSTRAINT lead_sync_outbox_lead_hash_fkey FOREIGN KEY (lead_hash) REFERENCES lead_state(lead_hash) ON DELETE CASCADE;
ALTER TABLE leads.lead_video_progress ADD CONSTRAINT lead_video_progress_lead_hash_fkey FOREIGN KEY (lead_hash) REFERENCES lead_state(lead_hash) ON DELETE CASCADE;
ALTER TABLE leads.nurture_runs ADD CONSTRAINT nurture_runs_project_key_sequence_key_fkey FOREIGN KEY (project_key, sequence_key) REFERENCES nurture_sequences(project_key, sequence_key) ON DELETE RESTRICT;
ALTER TABLE leads.nurture_subject_states ADD CONSTRAINT nurture_subject_states_project_key_sequence_key_fkey FOREIGN KEY (project_key, sequence_key) REFERENCES nurture_sequences(project_key, sequence_key) ON DELETE RESTRICT;

-- Sequenz-Eigentum (haelt DROP TABLE und Dump-Verhalten identisch zur Quelle).
ALTER SEQUENCE leads.quiz_sessions_id_seq OWNED BY leads.quiz_sessions.id;
ALTER SEQUENCE leads.tracking_sessions_id_seq OWNED BY leads.tracking_sessions.id;
ALTER SEQUENCE leads.tracking_events_id_seq OWNED BY leads.tracking_events.id;
ALTER SEQUENCE leads.tracking_video_progress_id_seq OWNED BY leads.tracking_video_progress.id;
ALTER SEQUENCE leads.lead_profiles_id_seq OWNED BY leads.lead_profiles.id;

-- Indexe (ohne die von Constraints automatisch erzeugten).
CREATE INDEX event_daily_member_day_idx ON leads_analytics.event_daily USING btree (member_id, event_day DESC);
CREATE INDEX event_daily_name_day_idx ON leads_analytics.event_daily USING btree (event_name, event_day DESC);
CREATE INDEX lead_contact_crm_berater_slug_idx ON leads.lead_contact_crm USING btree (berater_slug);
CREATE INDEX lead_contact_crm_linked_herbalife_id_idx ON leads.lead_contact_crm USING btree (linked_herbalife_id) WHERE (linked_herbalife_id IS NOT NULL);
CREATE INDEX lead_contact_crm_manual_added_idx ON leads.lead_contact_crm USING btree (member_id, manual_added) WHERE (manual_added = true);
CREATE INDEX lead_contact_crm_member_id_idx ON leads.lead_contact_crm USING btree (member_id);
CREATE INDEX lead_contact_crm_reminder_at_idx ON leads.lead_contact_crm USING btree (reminder_at) WHERE (reminder_active = true);
CREATE INDEX idx_le_lead_time ON leads.lead_events USING btree (lead_hash, event_at DESC);
CREATE INDEX idx_le_member_time ON leads.lead_events USING btree (member_id, event_at DESC);
CREATE INDEX idx_le_name_time ON leads.lead_events USING btree (event_name, event_at DESC);
CREATE INDEX idx_le_organisation_time ON leads.lead_events USING btree (organisation_id, event_at DESC);
CREATE INDEX idx_le_question_time ON leads.lead_events USING btree (question_ref, event_at DESC);
CREATE INDEX idx_le_ref_time ON leads.lead_events USING btree (ref_id, event_at DESC);
CREATE INDEX idx_le_video_time ON leads.lead_events USING btree (video_step, event_at DESC);
CREATE INDEX idx_lead_events_created_at_event_uid ON leads.lead_events USING btree (created_at DESC, event_uid);
CREATE INDEX idx_lead_events_event_at_desc ON leads.lead_events USING btree (event_at DESC);
CREATE INDEX idx_lmu_reason ON leads.lead_migration_unresolved USING btree (reason, created_at DESC);
CREATE INDEX idx_lmu_resolved_lead_hash ON leads.lead_migration_unresolved USING btree (resolved_lead_hash);
CREATE INDEX idx_lmu_session ON leads.lead_migration_unresolved USING btree (session_hash);
CREATE INDEX idx_lead_profiles_email_hash ON leads.lead_profiles USING btree (email_hash);
CREATE INDEX idx_lead_profiles_lead_hash ON leads.lead_profiles USING btree (lead_hash);
CREATE INDEX idx_lead_profiles_member_id ON leads.lead_profiles USING btree (member_id);
CREATE INDEX idx_lead_profiles_next_step ON leads.lead_profiles USING btree (next_step);
CREATE INDEX idx_lead_profiles_session_hash ON leads.lead_profiles USING btree (session_hash);
CREATE INDEX idx_lead_profiles_stage ON leads.lead_profiles USING btree (lifecycle_stage);
CREATE INDEX idx_lead_profiles_success_code ON leads.lead_profiles USING btree (success_code);
CREATE INDEX idx_ls_email_hash ON leads.lead_state USING btree (email_hash);
CREATE INDEX idx_ls_email_norm ON leads.lead_state USING btree (email_normalized);
CREATE INDEX idx_ls_last_event ON leads.lead_state USING btree (last_event_at DESC);
CREATE INDEX idx_ls_lifecycle_created ON leads.lead_state USING btree (lifecycle_stage, created_at DESC);
CREATE INDEX idx_ls_member_created ON leads.lead_state USING btree (member_id, created_at DESC);
CREATE INDEX idx_ls_organisation_created ON leads.lead_state USING btree (organisation_id, created_at DESC);
CREATE INDEX idx_ls_ref_created ON leads.lead_state USING btree (ref_id, created_at DESC);
CREATE INDEX idx_ls_utm_ad_id_created ON leads.lead_state USING btree (utm_ad_id, form_submitted_at DESC) WHERE (utm_ad_id IS NOT NULL);
CREATE INDEX idx_ls_utm_content_created ON leads.lead_state USING btree (utm_content, form_submitted_at DESC) WHERE (utm_content IS NOT NULL);
CREATE INDEX idx_lead_sync_outbox_lead_hash ON leads.lead_sync_outbox USING btree (lead_hash);
CREATE INDEX idx_outbox_locked ON leads.lead_sync_outbox USING btree (status, locked_at) WHERE (status = 'processing'::text);
CREATE INDEX idx_outbox_pending ON leads.lead_sync_outbox USING btree (status, next_attempt_at) WHERE (status = ANY (ARRAY['pending'::text, 'failed'::text]));
CREATE INDEX idx_lvp_lead_step ON leads.lead_video_progress USING btree (lead_hash, video_step);
CREATE INDEX idx_lvp_step_pct ON leads.lead_video_progress USING btree (video_step, max_unique_watched_percent);
CREATE INDEX nurture_runs_sequence_finished_idx ON leads.nurture_runs USING btree (project_key, sequence_key, finished_at DESC);
CREATE INDEX nurture_runs_status_finished_idx ON leads.nurture_runs USING btree (status, finished_at DESC);
CREATE INDEX nurture_subject_states_status_idx ON leads.nurture_subject_states USING btree (project_key, sequence_key, status, last_seen_at DESC);
CREATE INDEX idx_quiz_sessions_created_at ON leads.quiz_sessions USING btree (created_at);
CREATE INDEX idx_quiz_sessions_hash ON leads.quiz_sessions USING btree (hash);
CREATE INDEX idx_quiz_sessions_herbalife_id ON leads.quiz_sessions USING btree (herbalife_id);
CREATE INDEX idx_tracking_events_event_at ON leads.tracking_events USING btree (event_at);
CREATE INDEX idx_tracking_events_event_name ON leads.tracking_events USING btree (event_name);
CREATE INDEX idx_tracking_events_member_id ON leads.tracking_events USING btree (member_id);
CREATE INDEX idx_tracking_events_session_hash ON leads.tracking_events USING btree (session_hash);
CREATE INDEX idx_tracking_events_video ON leads.tracking_events USING btree (video_step, video_id);
CREATE INDEX idx_tracking_sessions_is_resume ON leads.tracking_sessions USING btree (is_resume);
CREATE INDEX idx_tracking_sessions_last_event ON leads.tracking_sessions USING btree (last_event_at);
CREATE INDEX idx_tracking_sessions_lead_hash ON leads.tracking_sessions USING btree (lead_hash);
CREATE INDEX idx_tracking_sessions_member_id ON leads.tracking_sessions USING btree (member_id);
CREATE INDEX idx_tracking_sessions_source_funnel ON leads.tracking_sessions USING btree (source_app, funnel);
CREATE INDEX idx_tracking_video_member_id ON leads.tracking_video_progress USING btree (member_id);
CREATE INDEX idx_tracking_video_session_hash ON leads.tracking_video_progress USING btree (session_hash);
CREATE INDEX idx_tracking_video_step ON leads.tracking_video_progress USING btree (video_step);
...
CREATE OR REPLACE FUNCTION leads_analytics.refresh_event_daily(p_from date, p_to date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
begin
  ...
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtext('leads_analytics.refresh_event_daily'));

  select count(*), min(event_at), max(event_at)
    into v_source_count, v_min_at, v_max_at
    from leads.lead_events
  ...
  insert into leads_analytics.event_daily (
  ...
  select coalesce(sum(event_count), 0), count(*)
    ...
    from leads_analytics.event_daily
   where event_day between p_from and p_to;
  ...
  insert into leads_analytics.refresh_runs (
...
CREATE OR REPLACE FUNCTION leads.analytics_dashboard_v2(p_from timestamp with time zone, p_to timestamp with time zone, p_member_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO ''
AS $function$
  ...
  from leads_analytics.event_daily d
...
CREATE OR REPLACE FUNCTION leads.analytics_events_page_v2(p_from timestamp with time zone, p_berater_slug text DEFAULT NULL::text, p_cursor_event_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_cursor_event_id bigint DEFAULT NULL::bigint, p_page_size integer DEFAULT 1000)
...
CREATE OR REPLACE FUNCTION leads.claim_outbox_jobs(worker_id text, batch_size integer DEFAULT 10)
 RETURNS SETOF lead_sync_outbox
 LANGUAGE plpgsql
 SET search_path TO 'leads', 'pg_temp'
AS $function$
BEGIN
  UPDATE leads.lead_sync_outbox
  SET
    status = 'failed',
...
CREATE OR REPLACE FUNCTION leads.enqueue_lead_sync(p_lead_hash text, p_sync_type text, p_context_data jsonb DEFAULT '{}'::jsonb)
 RETURNS bigint
 LANGUAGE plpgsql
 SET search_path TO 'leads', 'pg_temp'
AS $function$
...
BEGIN
  ...
  INSERT INTO leads.lead_sync_outbox (lead_hash, sync_type, context_data)
...
CREATE OR REPLACE FUNCTION leads.init_lead(p_client_seed uuid, p_lead_hash text DEFAULT NULL::text, p_member_id text DEFAULT NULL::text, p_organisation_id text DEFAULT NULL::text, p_ref_id text DEFAULT NULL::text, p_ref_type text DEFAULT NULL::text, p_berater_slug text DEFAULT NULL::text, p_source_app text DEFAULT 'business_leads_quiz'::text, p_funnel_key text DEFAULT 'business'::text, p_lang text DEFAULT NULL::text, p_country text DEFAULT NULL::text, p_utm_source text DEFAULT NULL::text, p_utm_medium text DEFAULT NULL::text, p_utm_campaign text DEFAULT NULL::text, p_utm_content text DEFAULT NULL::text, p_fbclid text DEFAULT NULL::text, p_fbc text DEFAULT NULL::text, p_fbp text DEFAULT NULL::text, p_event_source_url text DEFAULT NULL::text, p_utm_campaign_id text DEFAULT NULL::text, p_utm_adset_id text DEFAULT NULL::text, p_utm_ad_id text DEFAULT NULL::text, p_utm_term text DEFAULT NULL::text)
...
begin
  ...
  on conflict (client_seed) do update set
    last_seen_at = now(),
    member_id = coalesce(leads.lead_state.member_id, excluded.member_id),
    organisation_id = coalesce(leads.lead_state.organisation_id, excluded.organisation_id),
    ref_id = coalesce(leads.lead_state.ref_id, excluded.ref_id),
    ref_type = coalesce(leads.lead_state.ref_type, excluded.ref_type),
    berater_slug = coalesce(leads.lead_state.berater_slug, excluded.berater_slug),
    source_app = coalesce(leads.lead_state.source_app, excluded.source_app),
    funnel_key = coalesce(leads.lead_state.funnel_key, excluded.funnel_key),
    lang = coalesce(leads.lead_state.lang, excluded.lang),
    country = coalesce(leads.lead_state.country, excluded.country),
    utm_source = coalesce(leads.lead_state.utm_source, excluded.utm_source),
    utm_medium = coalesce(leads.lead_state.utm_medium, excluded.utm_medium),
    utm_campaign = coalesce(leads.lead_state.utm_campaign, excluded.utm_campaign),
    utm_content = coalesce(leads.lead_state.utm_content, excluded.utm_content),
    fbclid = coalesce(leads.lead_state.fbclid, excluded.fbclid),
    fbc = coalesce(leads.lead_state.fbc, excluded.fbc),
    fbp = coalesce(leads.lead_state.fbp, excluded.fbp),
    event_source_url = coalesce(leads.lead_state.event_source_url, excluded.event_source_url),
    utm_campaign_id = coalesce(leads.lead_state.utm_campaign_id, excluded.utm_campaign_id),
    utm_adset_id = coalesce(leads.lead_state.utm_adset_id, excluded.utm_adset_id),
    utm_ad_id = coalesce(leads.lead_state.utm_ad_id, excluded.utm_ad_id),
    utm_term = coalesce(leads.lead_state.utm_term, excluded.utm_term),
    updated_at = now()
  returning leads.lead_state.lead_hash;
end;
$function$
;

CREATE OR REPLACE FUNCTION leads.mark_outbox_done(p_job_id bigint, p_worker_id text, p_response_data jsonb DEFAULT '{}'::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SET search_path TO 'leads', 'pg_temp'
AS $function$
...
CREATE OR REPLACE FUNCTION leads.mark_outbox_failed(p_job_id bigint, p_worker_id text, p_error text, p_response_data jsonb DEFAULT '{}'::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SET search_path TO 'leads', 'pg_temp'
AS $function$
...
CREATE OR REPLACE FUNCTION leads.nurture_events_page(p_since timestamp with time zone, p_until timestamp with time zone, p_project_key text DEFAULT NULL::text, p_sequence_key text DEFAULT NULL::text, p_status text DEFAULT NULL::text, p_phase text DEFAULT NULL::text, p_limit integer DEFAULT 50, p_cursor_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_cursor_uid text DEFAULT NULL::text)
...
CREATE OR REPLACE FUNCTION leads.nurture_health_signals()
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO 'leads', 'pg_temp'
AS $function$
  with latest_run as (
    ...
    select max(signal_at) as event_at from (
      ...
      union all
      select max(last_seen_at) from leads.nurture_subject_states
    ) signals
  ), failures as (
...
CREATE OR REPLACE FUNCTION leads.nurture_overview(p_since timestamp with time zone, p_until timestamp with time zone, p_project_key text DEFAULT NULL::text, p_sequence_key text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO 'leads', 'pg_temp'
AS $function$
  with normalized as (
    ...
    select s.*,
      (select max(coalesce(r.finished_at, r.started_at)) from leads.nurture_runs r
       where r.project_key = s.project_key and r.sequence_key = s.sequence_key) as last_run_at,
      (select r.status from leads.nurture_runs r
    ...
    from leads.nurture_sequences s
    where s.active
...
CREATE OR REPLACE FUNCTION leads.nurture_people_page(p_since timestamp with time zone, p_until timestamp with time zone, p_project_key text DEFAULT NULL::text, p_sequence_key text DEFAULT NULL::text, p_journey text DEFAULT NULL::text, p_limit integer DEFAULT 50, p_cursor_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_cursor_ref text DEFAULT NULL::text)
...
CREATE OR REPLACE FUNCTION leads.record_nurture_failure(p_project_key text, p_sequence_key text, p_workflow_id text, p_workflow_version_id text, p_execution_id text, p_error_code text, p_error_node text, p_error_message text, p_occurred_at timestamp with time zone DEFAULT now())
 RETURNS text
 LANGUAGE plpgsql
 SET search_path TO 'leads', 'pg_temp'
AS $function$
...
begin
  ...
  perform leads.record_nurture_run(
...
begin
  ...
  on conflict (project_key, sequence_key, execution_id) do update set
    workflow_id = coalesce(excluded.workflow_id, leads.nurture_runs.workflow_id),
    workflow_version_id = coalesce(excluded.workflow_version_id, leads.nurture_runs.workflow_version_id),
    started_at = least(leads.nurture_runs.started_at, excluded.started_at),
    finished_at = coalesce(excluded.finished_at, leads.nurture_runs.finished_at),
...
CREATE OR REPLACE FUNCTION leads.record_nurture_sent(p_project_key text, p_sequence_key text, p_event_uid text, p_lead_hash text, p_phase_key text, p_email_id text DEFAULT NULL::text, p_subject text DEFAULT NULL::text, p_language text DEFAULT NULL::text, p_variant text DEFAULT NULL::text, p_mautic_contact_id text DEFAULT NULL::text, p_occurred_at timestamp with time zone DEFAULT now(), p_telemetry_version integer DEFAULT 2)
 RETURNS text
 LANGUAGE plpgsql
 SET search_path TO 'leads', 'pg_temp'
AS $function$
...
begin
  ...
  insert into leads.nurture_subject_states (
  ...
  on conflict (project_key, sequence_key, lead_hash) do update set
    ...
    first_seen_at = case when leads.nurture_subject_states.status = 'sent'
      then leads.nurture_subject_states.first_seen_at else excluded.first_seen_at end,
    last_seen_at = greatest(leads.nurture_subject_states.last_seen_at, excluded.last_seen_at),
    status_changed_at = case when leads.nurture_subject_states.status = 'sent'
      then leads.nurture_subject_states.status_changed_at else excluded.status_changed_at end,
    occurrence_count = case when leads.nurture_subject_states.status = 'sent'
      then leads.nurture_subject_states.occurrence_count + 1 else 1 end,
...
CREATE OR REPLACE FUNCTION leads.record_nurture_skip(p_project_key text, p_sequence_key text, p_lead_hash text, p_reason_code text, p_phase_key text DEFAULT NULL::text, p_occurred_at timestamp with time zone DEFAULT now(), p_telemetry_version integer DEFAULT 2)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'leads', 'pg_temp'
AS $function$
declare
  v_existing leads.nurture_subject_states;
  v_state leads.nurture_subject_states;
...
begin
  ...
  from leads.nurture_subject_states
  ...
  on conflict (project_key, sequence_key, lead_hash) do update set
    ...
    first_seen_at = case
      when leads.nurture_subject_states.status = excluded.status
        and leads.nurture_subject_states.reason_code is not distinct from excluded.reason_code
        and leads.nurture_subject_states.phase_key is not distinct from excluded.phase_key
      then leads.nurture_subject_states.first_seen_at else excluded.first_seen_at end,
    last_seen_at = greatest(leads.nurture_subject_states.last_seen_at, excluded.last_seen_at),
    ...
    occurrence_count = case
      when leads.nurture_subject_states.status = excluded.status
        ...
        and leads.nurture_subject_states.phase_key is not distinct from excluded.phase_key
      then leads.nurture_subject_states.occurrence_count + 1 else 1 end,
    last_event_uid = case when v_state_changed then excluded.last_event_uid else leads.nurture_subject_states.last_event_uid end,
    telemetry_version = excluded.telemetry_version,
    updated_at = now()
...
CREATE OR REPLACE FUNCTION leads.submit_lead_complete(p_state jsonb, p_answers jsonb DEFAULT '[]'::jsonb, p_lang text DEFAULT NULL::text, p_answered_at timestamp with time zone DEFAULT now())
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'leads', 'pg_temp'
AS $function$
...
BEGIN
  ...
  WHERE NOT EXISTS (
    ...
    WHERE c.table_schema = 'leads' AND c.table_name = 'lead_state' AND c.column_name = t.k
  );
  ...
  EXECUTE format(
    'INSERT INTO leads.lead_state (%s) '
    || 'SELECT %s FROM jsonb_populate_record(NULL::leads.lead_state, $1) AS r '
  ...
  FOR v_answer IN SELECT value FROM jsonb_array_elements(coalesce(p_answers, '[]'::jsonb)) LOOP
    PERFORM leads.upsert_answer_current(
      v_lead_hash,
...
BEGIN
  ...
  INSERT INTO leads.lead_answers_current (
    lead_hash,
    question_ref,
    question_index,
    question_text,
...
BEGIN
  ...
  FROM leads.v_lead_state_full v
  ...
  FROM leads.lead_video_progress
  ...
  INSERT INTO leads.lead_video_progress (
    lead_hash,
    video_step,
    video_id,
    duration_seconds,
  ...
  ON CONFLICT (lead_hash, video_step) DO UPDATE SET
    video_id = COALESCE(leads.lead_video_progress.video_id, EXCLUDED.video_id),
    duration_seconds = GREATEST(
      leads.lead_video_progress.duration_seconds,
      EXCLUDED.duration_seconds
    ),
    max_unique_watched_percent = GREATEST(
      leads.lead_video_progress.max_unique_watched_percent,
      EXCLUDED.max_unique_watched_percent
    ),
    max_playhead_percent = GREATEST(
      leads.lead_video_progress.max_playhead_percent,
      EXCLUDED.max_playhead_percent
    ),
    unique_watched_seconds = GREATEST(
      leads.lead_video_progress.unique_watched_seconds,
      EXCLUDED.unique_watched_seconds
    ),
    seek_count = GREATEST(
      leads.lead_video_progress.seek_count,
      EXCLUDED.seek_count
    ),
    completed_at = COALESCE(
      leads.lead_video_progress.completed_at,
      CASE
        WHEN GREATEST(
          leads.lead_video_progress.max_unique_watched_percent,
          EXCLUDED.max_unique_watched_percent
  ...
  IF v_after_rank > v_before_rank AND v_after_rank > 0 THEN
    PERFORM leads.enqueue_lead_sync(
      p_lead_hash,
      'mysql_rank_update',
...
-- Views.
CREATE VIEW leads.v_nurture_runs_wahr AS
SELECT id,
    project_key,
    sequence_key,
    execution_id,
...
CREATE VIEW leads.v_sync_dead_jobs AS
SELECT id,
    lead_hash,
    sync_type,
    context_data,
...
CREATE VIEW leads.v_funnel_analysis AS
SELECT berater_slug,
...
CREATE VIEW leads.v_resume_metrics AS
SELECT berater_slug,
    count(
        CASE
...
CREATE VIEW leads.v_completion_metrics AS
SELECT berater_slug,
...
CREATE VIEW leads.v_lead_state_full AS
SELECT ls.lead_hash,
    ls.client_seed,
    ls.member_id,
    ls.ref_id,
...
-- Trigger.
CREATE TRIGGER trg_lead_answers_current_updated_at BEFORE UPDATE ON leads.lead_answers_current FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER trg_lead_migration_unresolved_updated_at BEFORE UPDATE ON leads.lead_migration_unresolved FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER trg_lead_state_updated_at BEFORE UPDATE ON leads.lead_state FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER trg_lead_sync_outbox_updated_at BEFORE UPDATE ON leads.lead_sync_outbox FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER trg_lead_video_progress_updated_at BEFORE UPDATE ON leads.lead_video_progress FOR EACH ROW EXECUTE FUNCTION set_updated_at();
