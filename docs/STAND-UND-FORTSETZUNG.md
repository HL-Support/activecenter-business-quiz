...
| A4 | **Business-Kalkulator** (`herbalife-erfolgs-berechner`) auf Coolify **und** Postgres | Datenseite ✅ **erledigt 28.08.**, Anwendung offen — siehe A4a bis A4c |
...
| A4c | **Anwendung umstellen** — Vercel/Supabase → Coolify/Postgres | 🔴 **Vercel kann die Plattform-DB gar nicht erreichen** (`pg_hba` lässt nur `10.0.1.5` zu, den Coolify-Host). Der Umzug auf Coolify ist deshalb **Voraussetzung**, nicht Alternative. Günstige Lage: **alle** Supabase-Zugriffe laufen durch **zwei** Funktionen (`api/contacts.js:166`, `api/sso.js:12`) — der erprobte Übersetzer `server/postgrest-nach-sql.js` + `server/db-transport.js` aus diesem Repo ist wiederverwendbar, die 19 Aufrufstellen bleiben unangetastet |
