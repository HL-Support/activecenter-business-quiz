...
| A4 | **Business-Kalkulator** (`herbalife-erfolgs-berechner`) auf Coolify **und** Postgres | Datenseite ✅ **erledigt 28.08.**; Anwendung ✅ **Schritte 1–5 erledigt 29.08.**: läuft als Coolify-App `kalkulator` unter `kalkulator.hl-support.biz` im **direkten Modus** (eigene Rolle `leads_kalk`), Vergleichslauf bestanden. 🔴 Dabei gemessen: **CRM-Speichern auf Vercel ist seit der Schreibbarriere kaputt** (Supabase 42501 auf `lead_contact_crm`) — der Umstieg repariert es. Offen ist nur Schritt 6 (Markus: Domain-Segen; Laravel-SSO-Ziel umstellen; SSO-Delta erneut messen — Stand 29.08. früh: **0**; Vercel stilllegen). Runbook: **Business_Kalkulator/UMZUG-COOLIFY-POSTGRES.md §5b** |
