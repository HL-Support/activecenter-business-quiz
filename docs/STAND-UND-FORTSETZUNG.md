...
| A4d | 🔴 **Zwei Lücken im Übersetzer, vor allem anderen zu schliessen** | Der Übersetzer wurde gegen die **echten** 14 Abfragen des Kalkulators laufen gelassen: 12 gehen durch, **zwei nicht** — und beide treffen die Kontaktliste (`api/contacts.js:1138`): `email=not.is.null` → „Unbekannter Operator", `offset=60` → „offset wird nicht unterstuetzt (Blaetterung ueber Keyset)". Erweiterung gehört ins **Quiz-Repo**, mit Tests, und wird von dort übernommen |
