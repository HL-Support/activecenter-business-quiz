...
def bulletproof_btn(url, label, accent, btn_text):
    return (
        ...
        f'style="height:50px;v-text-anchor:middle;width:320px;" arcsize="14%" stroke="f" fillcolor="{accent}">'
        ...
        f'padding:14px 32px;text-align:center;text-decoration:none;white-space:nowrap;-webkit-text-size-adjust:none;">{label}</a>'
...
def preprocess(text):
    text = text.strip()
    text = re.sub(r'^(Hallo|Ciao) \{contactfield=firstname\},\s*\n+', '', text)
