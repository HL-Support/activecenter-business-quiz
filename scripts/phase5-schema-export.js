...
//
// SCHEMA-ABBILDUNG (seit 27.08. abends): Die erzeugte DDL traegt die PLATTFORM-Schemata
// (leads, leads_analytics) statt der Quellschemata - Abbildung und Restkontrolle in
// phase5-schema-abbildung.js. Der Import beginnt mit SET LOCAL ROLE leads_owner
// (Rollenmodell-Vertrag). Der Paritaets-Schnappschuss bleibt in QUELL-Namen: er ist
// der Schnappschuss der Quelle; der Vergleich bildet selbst ab.
//
//   node --env-file=.env.prod scripts/phase5-schema-export.js
//     -> docs/audits/cutover-vorbereitung/phase5-testimport/schema-<datum>-leads.sql
//     -> docs/audits/cutover-vorbereitung/phase5-testimport/paritaet-live-<datum>-leads.json
// (Suffix -leads: die unabgebildeten Artefakte des ersten Testimports vom 27.08.
//  bleiben als Beweis unangetastet.)

const fs = require('fs');
const path = require('path');
const { executeManagementQuery } = require('./stats-logs-baseline.js');
const { bildeSchemaAb, bildeDefinitionAb, pruefeRestfrei } = require('./phase5-schema-abbildung.js');
...
async function main() {
  ...
  zeilen.push('-- Schemata sind auf die Plattform abgebildet (Quelle public -> leads,');
  zeilen.push('-- Quelle analytics_internal -> leads_analytics); Restkontrolle erzwungen.');
  zeilen.push('');
  zeilen.push('\\set ON_ERROR_STOP on');
  zeilen.push('BEGIN;');
  zeilen.push('');
  zeilen.push('-- Superuser-Teil. AUTHORIZATION haelt das Schema-Eigentum bei leads_owner,');
  zeilen.push('-- auch wenn ein Superuser den Import faehrt. pgcrypto: Paritaet zur Quelle;');
  zeilen.push('-- die Funktionsruempfe selbst brauchen nur pg_catalog (gen_random_uuid, hashtext).');
  zeilen.push('CREATE EXTENSION IF NOT EXISTS pgcrypto;');
  zeilen.push('CREATE SCHEMA IF NOT EXISTS leads AUTHORIZATION leads_owner;');
  zeilen.push('CREATE SCHEMA IF NOT EXISTS leads_analytics AUTHORIZATION leads_owner;');
  zeilen.push('');
  zeilen.push('-- Rollenmodell-Vertrag: jede Migration beginnt mit SET ROLE leads_owner -');
  zeilen.push('-- sonst gehoeren die Objekte dem Zugang und die Standardrechte greifen nicht.');
  zeilen.push('SET LOCAL ROLE leads_owner;');
  zeilen.push('-- Unqualifizierte Verweise in View-/Constraint-/Trigger-Definitionen loesen');
  zeilen.push('-- ueber den search_path in die Zielschemata auf.');
  zeilen.push('SET LOCAL search_path TO leads, leads_analytics;');
  zeilen.push('');
  ...
  for (const s of auswahlSequenzen) {
    zeilen.push(`CREATE SEQUENCE ${bildeSchemaAb(s.schema)}.${s.name} AS ${s.typ} INCREMENT BY ${s.increment_by} MINVALUE ${s.min_value} MAXVALUE ${s.max_value} START WITH ${s.start_value} CACHE ${s.cache_size}${s.cycle ? ' CYCLE' : ''};`);
  }
  zeilen.push('');

  for (const [schema, tabelle] of TABELLEN) {
    const cols = spalten.filter((z) => z.schema === schema && z.tabelle === tabelle);
    zeilen.push(`CREATE TABLE ${bildeSchemaAb(schema)}.${tabelle} (`);
    zeilen.push(cols.map((c) => {
      ...
      else if (c.default_ausdruck) def += ` DEFAULT ${bildeDefinitionAb(c.default_ausdruck)}`;
  ...
  for (const c of constraints) {
    zeilen.push(`ALTER TABLE ${bildeSchemaAb(c.schema)}.${c.tabelle} ADD CONSTRAINT ${c.name} ${bildeDefinitionAb(c.definition)};`);
  }
  zeilen.push('');
  ...
  for (const s of auswahlSequenzen) {
    ...
    zeilen.push(`ALTER SEQUENCE ${bildeSchemaAb(s.schema)}.${s.name} OWNED BY ${bildeDefinitionAb(ziel)}.${s.gehoert_zu_spalte};`);
  }
  zeilen.push('');
  ...
  for (const i of indexe) {
    ...
    zeilen.push(`${bildeDefinitionAb(i.definition)};`);
  }
  zeilen.push('');
  ...
  for (const f of funktionen) {
    zeilen.push(`${bildeDefinitionAb(f.definition)};`);
    zeilen.push('');
  }
  ...
  for (const v of sortiert) {
    zeilen.push(`CREATE VIEW ${bildeSchemaAb(v.schema)}.${v.name} AS`);
    zeilen.push(bildeDefinitionAb(v.definition.trim().replace(/;$/, '')) + ';');
    zeilen.push('');
  }

  zeilen.push('-- Trigger.');
  for (const t of trigger) {
    zeilen.push(`${bildeDefinitionAb(t.definition)};`);
  }
  zeilen.push('');
  zeilen.push('COMMIT;');
  zeilen.push('');

  const sql = zeilen.join('\n');
  // Restkontrolle: kein Quellschema-Token darf die Abbildung ueberleben (harter Fehler).
  pruefeRestfrei(sql, 'schema-export');
  const sqlPfad = path.join(zielDir, `schema-${datum}-leads.sql`);
  fs.writeFileSync(sqlPfad, sql, 'utf8');
  ...
  const jsonPfad = path.join(zielDir, `paritaet-live-${datum}-leads.json`);
